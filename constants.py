...
# @Last Modified time: 2021-11-01 16:32:01
...
BASELINE_QUANTILE = 0.08  # quantile used for the computation of the fluorescence baseline (value taken from https://www.researchgate.net/post/Calculating-dF-F-for-calcium-signaling-data)
BASELINE_WINDOW_SIZE = 10.  # default window length (in s) to compute the fluorescence baseline  
...
DFF_OUTLIER = 0.3  # upper bound threshold for df/f (cells with absolute traces above this threshold get discarded). 0.8 works well for line3
ROI_KEY = 'ROI'  # key used to access list of ROIs from suite2p output dictionaries
